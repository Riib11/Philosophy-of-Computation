...
But is this how the CNN learned to play? Are there analogous concepts of \textit{ball}, \textit{paddle}, \textit{brick}, and \textit{tunnel} represented within the neural network?
...
Recalling the Breakout case, the obvious question to ask of the CNN is ``if the CNN didn't learn even basic abstractions like \textit{the paddle}, then what \textit{did} the CNN learn that allowed it to play so amazing well?'' While it is possible that CNN learned a few human-recognizable abstractions, the fact that it misses many that humans would find fundamental to undestanding (e.g. the abstraction of \textit{the paddle}) suggests that the CNN probably didn't learn many.
...
A simple and likely explanation for the CNN's behavior that accounts for its pattern-obliviousness is that it discovered a sort of ``cheat'' in the form of statistical correlations between certain inputs (pixels and game moves) that were easier to learn that the highe-level abstractions that humans find intuitive and typically expect from a good player (human or otherwise).
The fact that the first significant attempts at designing intelligent systems was a symbolic approach, where human-intuitive patterns were \textit{card-coded} into the system. For example. early chess-player AIs needed hueristics such as ``treat the queen as having a relative value of 9 when evaluating piece trade-offs'' explicity represented in their desicion function).

In the previous section on learning Breakout, a human reader would find it obvious that a human player can skip many of the early steps of learning to play breakout. These include

In the previous section when the steps of learning how to play Breakout was touched on, it should have been clear that humans can skip many of the early steps because humans have an intuitive sense of physics and object-ness. Humans know how to expect the ball to bounce around the screen even if they have never looked at a Breakout game. Additionally, humans look at the paddle as an \textit{object} that can possibly move vertically and still remain the same object, as well as the usual left/right. (The question of how or whether humans learn these intuitions, as I describe them, in the first place is an intriguing question that will be addressed later.)
